...
expty TransVar(S_table venv, S_table tenv, A_var var) { //对变量进行类型检查
	switch (var->kind) {
	...
	case A_fieldVar: {    //有难度，想了好久，未测试
		expty et = TransVar(venv, tenv, var->u.field.var);
		Ty_ty ty = et->ty;
	...
	case A_subscriptVar: { //类似 A_fieldVar 只是检查前面是不是数组类型的变量, 还需要检查 [] 内的是不是整数类型
		expty et1 = TransVar(venv, tenv, var->u.subscript.var);
		expty et2 = TransExp(venv, tenv, var->u.subscript.exp);
...
expty TransExp(S_table venv, S_table tenv, A_exp exp) { //对表达式进行类型检查
	switch (exp->kind) {
	...
	case A_callExp: {  //函数的类型检查有难度
		...
		if (e && e->kind == E_funEntry) {
			...
			while (tl) {  ///函数的类型检查的关键部分 →  
				...
				if (TransExp(venv, tenv, exp_)->ty->kind != Actual_Ty(t)->kind) {
	...
	case A_opExp: {  //可以检查记录和数组类型
		expty et1 = TransExp(venv, tenv, exp->u.op.exp1);
		expty et2 = TransExp(venv, tenv, exp->u.op.exp2);
	...
	case A_recordExp: { //有难度，和函数类型检查类似
		...
		if (ty && ty->kind == Ty_record) {
			...
			while (tfl) {  ///记录的类型检查的关键部分 →  
				...
				else if (TransExp(venv, tenv, af->exp)->ty->kind != Actual_Ty(tf->ty)->kind)
	...
	case A_assignExp: {
		expty et1 = TransVar(venv, tenv, exp->u.assign.var);
		expty et2 = TransExp(venv, tenv, exp->u.assign.exp);
	...
	case A_ifelseExp: {
		if (TransExp(venv, tenv, exp->u.ifelse.exp1)->ty->kind != Ty_int)
			EM_error(exp->u.ifelse.exp1->pos, "exp after 'if' must be an integer");
		expty et1 = TransExp(venv, tenv, exp->u.ifelse.exp2);
		expty et2 = TransExp(venv, tenv, exp->u.ifelse.exp3);
	...
	case A_forExp: {  ///有隐藏声明，有点特殊，需要修改
		...
		if (TransExp(venv, tenv, exp->u.for_.exp1)->ty->kind != Ty_int)
			EM_error(exp->u.for_.exp1->pos, "exp must be an integer");
		if (TransExp(venv, tenv, exp->u.for_.exp2)->ty->kind != Ty_int)
			EM_error(exp->u.for_.exp2->pos, "exp must be an integer");

		S_beginScope(venv);
		S_enter(venv, exp->u.for_.id, E_VarEntry(Ty_Int()));
		if (TransExp(venv, tenv, exp->u.for_.exp3)->ty->kind != Ty_void)
	...
	case A_arrayExp: {
		...
		if (ty->kind == Ty_array) {
			if (TransExp(venv, tenv, exp->u.array.exp1)->ty->kind != Ty_int)
				EM_error(exp->u.array.exp1->pos, "exp in '[]' must be an integer");
			if (TransExp(venv, tenv, exp->u.array.exp2)->ty->kind != Actual_Ty(ty->u.array)->kind)
...
void TransDec(S_table venv, S_table tenv, A_dec dec) {  //所有声明表达式均没有返回值类型
	switch (dec->kind) {

	case A_varDec: {
		expty et = TransExp(venv, tenv, dec->u.var.exp);
		if (dec->u.var.type == NULL) {
			if (et->ty->kind == Ty_nil)
				EM_error(dec->pos, "there must be a type reference to a nil exp");
			else
				S_enter(venv, dec->u.var.id, E_VarEntry(et->ty));
		}
		else {
			...
			if (!ty)
				...
				S_enter(venv, dec->u.var.id, E_VarEntry(ty));
			else
	...
	case A_functionDec: { //还不能处理递归函数声明
		...
		S_enter(venv, dec->u.function.func, E_FunEntry(tl, ty));
		S_beginScope(venv);
		A_tyfieldList tfl = dec->u.function.tyfieldList;
		while (tfl && tl) {
			S_enter(venv, tfl->head->id, E_VarEntry(tl->head));
			tfl = tfl->tail; tl = tl->tail;
		}
		expty et = TransExp(venv, tenv, dec->u.function.exp);
		if(ty)
