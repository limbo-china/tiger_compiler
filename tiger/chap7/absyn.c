...
A_exp A_IfElseExp(A_pos pos, A_exp exp1, A_exp exp2, A_exp exp3) {
	A_exp e = checked_malloc(sizeof(*e));
	e->kind = A_ifelseExp;e->pos = pos;e->u.ifelse.exp1 = exp1;e->u.ifelse.exp2 = exp2;e->u.ifelse.exp3 = exp3;
	return e;
}
A_exp A_IfExp(A_pos pos, A_exp exp1, A_exp exp2) {
	A_exp e = checked_malloc(sizeof(*e));
	e->kind = A_ifExp;e->pos = pos;e->u.if_.exp1 = exp1;e->u.if_.exp2 = exp2;
	return e;
}
...
A_dec A_FunctionDec(A_pos pos, S_symbol func, A_tyfieldList tflist, S_symbol type, A_exp exp/*, U_boolList formals*/) {
	...
	//d->u.function.formals = formals;
	return d;
}
A_dec A_TypeDec(A_pos pos, S_symbol type, A_ty ty) {
	A_dec d = checked_malloc(sizeof(*d));
	d->kind = A_typeDec;d->pos = pos;d->u.type.name = type;d->u.type.ty = ty;
	return d;
}
...
A_expList A_ConvertAssignEl(A_assignfieldList afl) {
	if (!afl)
		return (void*)0; //´úÌæ return NULL;
	return A_ExpList(afl->head->exp, A_ConvertAssignEl(afl->tail));
}
