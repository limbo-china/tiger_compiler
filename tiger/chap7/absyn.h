...
struct A_exp_ {
	enum {
		...
		A_seqExp, A_assignExp, A_ifelseExp, A_ifExp, A_whileExp, A_breakExp, A_forExp, A_letExp, A_arrayExp
	} kind;
	A_pos pos;
	union {
		A_var var;
		...
		struct { A_exp exp1, exp2, exp3; } ifelse;  
		struct { A_exp exp1, exp2; } if_;	//·ÀÖ¹ÃüÃû³åÍ»
...
struct A_dec_ {
	...
	A_pos pos;
	union {
		struct { S_symbol func; A_tyfieldList tyfieldList; S_symbol type; A_exp exp; /*U_boolList formals;*/ } function;
		struct { S_symbol id; S_symbol type; A_exp exp; /*bool escape;*/ } var;
		struct { S_symbol name; A_ty ty; } type;
	}u;
};

struct A_ty_ {
	enum { A_nameTy, A_tyfieldTy, A_arrayTy } kind;
	A_pos pos;
	union {
		S_symbol name;
...
A_exp A_IfElseExp(A_pos pos, A_exp exp1, A_exp exp2, A_exp exp3);
A_exp A_IfExp(A_pos pos, A_exp exp1, A_exp exp2);
...
A_dec A_FunctionDec(A_pos pos, S_symbol func, A_tyfieldList tflist, S_symbol type, A_exp exp/*, U_boolList formals*/);
A_dec A_VarDec(A_pos pos, S_symbol id, S_symbol type, A_exp exp/*, bool escape*/);
A_dec A_TypeDec(A_pos pos, S_symbol type, A_ty ty);

A_ty A_NameTy(A_pos pos, S_symbol type);
...
A_expList A_ConvertAssignEl(A_assignfieldList afl);
#endif
