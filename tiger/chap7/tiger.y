...
exp:   	 		
	...
	|	seq			{$$=$1;}
	|	MINUS INT  %prec UMINUS		{$$=A_OpExp(EM_tokPos,A_IntExp(EM_tokPos,0),A_minusOp,A_IntExp(EM_tokPos,$2));}	//表示为(0-i)的操作符表达式
	|	funcall		{$$=$1;}
	|	exp PLUS exp	{$$=A_OpExp(EM_tokPos,$1,A_plusOp,$3);}
	|	exp MINUS exp 	{$$=A_OpExp(EM_tokPos,$1,A_minusOp,$3);}
	|	exp TIMES exp 	{$$=A_OpExp(EM_tokPos,$1,A_timesOp,$3);}
	|	exp DIVIDE exp 	{$$=A_OpExp(EM_tokPos,$1,A_divideOp,$3);}
	|	exp EQ exp 	    {$$=A_OpExp(EM_tokPos,$1,A_eqOp,$3);}
	|	exp NEQ exp		{$$=A_OpExp(EM_tokPos,$1,A_neqOp,$3);}
	|	exp LT exp 		{$$=A_OpExp(EM_tokPos,$1,A_ltOp,$3);}
	|	exp LE exp 		{$$=A_OpExp(EM_tokPos,$1,A_leOp,$3);}
	|	exp GT exp 		{$$=A_OpExp(EM_tokPos,$1,A_gtOp,$3);}
	|	exp GE exp 		{$$=A_OpExp(EM_tokPos,$1,A_geOp,$3);}
	|	exp AND exp		{$$=A_OpExp(EM_tokPos,$1,A_andOp,$3);}
	|	exp OR exp		{$$=A_OpExp(EM_tokPos,$1,A_orOp,$3);}
	|	record		{$$=$1;}
	|	array		{$$=$1;}
	|	lvalue ASSIGN exp	{$$=A_AssignExp(EM_tokPos,$1,$3);}
	|	IF exp THEN exp ELSE exp		{$$=A_IfElseExp(EM_tokPos,$2,$4,$6);}
